...
class _$AppDatabase extends AppDatabase {
  ...
  ExpenseDao? _expensesDaoInstance;
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `expenses` (`id` INTEGER, `amount` INTEGER, `category` TEXT, `date` TEXT, `description` TEXT, PRIMARY KEY (`id`))');
  ...
  ExpenseDao get expensesDao {
    return _expensesDaoInstance ??= _$ExpenseDao(database, changeListener);
  }
}
...
class _$ExpenseDao extends ExpenseDao {
  ...
  @override
  Future<List<ExpenseModel>> getRecentExpenses(int n) async {
    return _queryAdapter.queryList(
        'SELECT * FROM expenses SORT BY date DESC LIMIT ?1',
        mapper: (Map<String, Object?> row) => ExpenseModel(
            id: row['id'] as int?,
            amount: row['amount'] as int?,
            category: row['category'] as String?,
